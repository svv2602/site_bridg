...
import { saveJob, updateJob, cleanupOldJobs, type JobStatus } from '../endpoints/jobStore';
...
// ---- Retry configuration for cron tasks ----

const CRON_RETRY_CONFIG = {
  maxRetries: 2, // Up to 2 retries (3 total attempts)
  retryDelayMs: 30 * 60 * 1000, // 30 minutes between retries
};

const taskRetryState = new Map<string, { retries: number; timer?: ReturnType<typeof setTimeout> }>();

/**
 * Run a task callback with automatic retry on failure.
 * If the task fails, schedule a retry after 30 minutes (max 2 retries).
 */
async function runWithRetry(taskId: string, callback: () => Promise<void>): Promise<void> {
  const state = taskRetryState.get(taskId) || { retries: 0 };

  try {
    await callback();
    // Success — reset retry state
    taskRetryState.delete(taskId);
  } catch (error) {
    const errorMsg = error instanceof Error ? error.message : String(error);

    if (state.retries < CRON_RETRY_CONFIG.maxRetries) {
      state.retries++;
      schedulerLogger.warn(
        `Task "${taskId}" failed (attempt ${state.retries}/${CRON_RETRY_CONFIG.maxRetries + 1}), ` +
        `retrying in ${CRON_RETRY_CONFIG.retryDelayMs / 60000} minutes: ${errorMsg}`
      );
      ...
      taskRetryState.set(taskId, state);
    } else {
      schedulerLogger.error(
        `Task "${taskId}" failed after ${state.retries + 1} attempts: ${errorMsg}`
      );
      taskRetryState.delete(taskId);
    }
  }
...
function startTaskCron(taskId: string): void {
  ...
  if (!cron.validate(config.cronExpression)) {
    schedulerLogger.error(`Invalid cron expression for "${taskId}": ${config.cronExpression}`);
    return;
  }
  ...
  schedulerLogger.info(`Cron started for "${taskId}": "${config.cronExpression}" (${config.timezone})`);
}
...
async function runScheduledPipeline(): Promise<void> {
  ...
  schedulerLogger.info(`Cron triggered pipeline job ${jobId}`);
  ...
  updateJob(job);
  schedulerLogger.info(`Pipeline completed: ${jobId}`);
}
...
async function runScheduledSmartArticles(): Promise<void> {
  ...
  schedulerLogger.info(`Cron triggered smart-articles job ${jobId}`);
  ...
  try {
    ...
    updateJob(job);
    schedulerLogger.info(`Smart articles completed: ${jobId}`);
  } catch (error: unknown) {
    const err = error as { message: string; stdout?: string };
    job.status = 'failed';
    job.completedAt = new Date().toISOString();
...
export function initScheduler(): void {
  try {
    ...
    schedulerLogger.info(`Initialized tasks: ${summary}`);

    // Cleanup old completed/failed jobs (older than 30 days)
    const cleaned = cleanupOldJobs(30);
    if (cleaned > 0) {
